...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # Statistics for long-tail analysis (Synchronous mode)
        self.total_long_probes = 0
        self.total_long_matches_sum = 0.0
        self.long_tail_threshold = 512
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        try:
                            ...
                            num_samples = batch_size // n_rollouts
                            ...
                            all_responses = self.tokenizer.batch_decode(gen_batch_output.batch['responses'].cpu(), skip_special_tokens=True)

                            for i in range(num_samples):
                                start_idx = i * n_rollouts
                                end_idx = (i + 1) * n_rollouts
                                
                                sample_lengths = all_lengths[start_idx:end_idx]
                                sample_responses = all_responses[start_idx:end_idx]
                                sample_id = f"step_{self.global_steps}_sample_{i}"

                                # Long-tail Analysis
                                if len(sample_lengths) > 1:
                                    is_first_long = sample_lengths[0] > self.long_tail_threshold
                                    if is_first_long:
                                        self.total_long_probes += 1
                                        rest_lengths = sample_lengths[1:]
                                        long_rest_count = sum(1 for l in rest_lengths if l > self.long_tail_threshold)
                                        self.total_long_matches_sum += (long_rest_count / len(rest_lengths))
