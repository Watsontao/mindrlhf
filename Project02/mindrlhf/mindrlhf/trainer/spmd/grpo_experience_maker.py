...
from mindrlhf.trainer.spmd.segment_rollout_manager import SegmentRolloutManager
from mindrlhf.tools.runtime_profiler import RuntimeProfiler
from mindspore.communication import get_rank
...
class GRPOExperienceMaker:
    ...
    def __init__(
        ...
        self.segment_rollout_manager = SegmentRolloutManager(
            self.grpo_config.rl_config, self.grpo_config.generate_config.sampling_config
        )
        self.segment_rollout_metrics = {}
        log_dir = getattr(self.grpo_config.rl_config, "save_prompt_completions_dir", ".") or "."
        log_dir = os.path.abspath(log_dir)
        os.makedirs(log_dir, exist_ok=True)
        self.segment_rollout_log_path = os.path.join(log_dir, "segment_rollout_metrics.log")
        try:
            with open(self.segment_rollout_log_path, "w", encoding="utf-8") as file:
                file.write("")
        except OSError as exc:
            logger.warning(f"Failed to reset segment rollout metrics log: {exc}")
        runtime_cfg = getattr(self.grpo_config, "runtime_profile_config", None)
        try:
            rank_id = get_rank()
        except RuntimeError:
            rank_id = None
        if runtime_cfg:
            self.runtime_profiler = RuntimeProfiler(
                enable=runtime_cfg.enable,
                log_dir=runtime_cfg.log_dir,
                record_gpu_memory=runtime_cfg.record_gpu_memory,
                record_host_memory=runtime_cfg.record_host_memory,
                step_log_interval=runtime_cfg.step_log_interval,
                epoch_log_interval=runtime_cfg.epoch_log_interval,
                rank_id=rank_id,
            )
        else:
            self.runtime_profiler = RuntimeProfiler(enable=False)
    ...
    def generate_sequence(self, micro_bs, micro_num, prompt_tensors_full):
        ...
        def _run_infer(input_batch, max_tokens):
            ...
            logger.info(f"generate sequence results type {type(results)}")
    ...
    def _append_segment_rollout_log(self):
        ...
        if not self.segment_rollout_metrics or not self.segment_rollout_log_path:
            return
        if get_rank() not in (None, 0):
            return
        entry = {"step": int(self.make_exp_step), **self.segment_rollout_metrics}
        try:
            with open(self.segment_rollout_log_path, "a", encoding="utf-8") as file:
                file.write(json.dumps(entry, ensure_ascii=False) + "\n")
        except OSError as exc:
            logger.warning(f"Failed to write segment rollout metrics log: {exc}")
    ...
    def make_experience(self, num_rollouts: int = 1, num_generations: int = 16):
        ...
        with self.runtime_profiler.profile("make_experience", {"step": int(self.make_exp_step)}):
            ...
            with self.runtime_profiler.profile("generation", {"step": int(self.make_exp_step)}):
            ...
            with TimeConsumingCollector("calculate reward"), self.runtime_profiler.profile(
                "reward", {"step": int(self.make_exp_step)}
            ):
            ...
            if self.segment_rollout_metrics:
                metrics.update(self.segment_rollout_metrics)
                logger.info(f"Segment rollout metrics: {self.segment_rollout_metrics}")
                self._append_segment_rollout_log()
            ...
            if self.grpo_config.rl_config.enable_ref:
                ...
                with self.runtime_profiler.profile("ref_log_prob", {"step": int(self.make_exp_step)}):
            ...
            if self.grpo_config.rl_config.enable_oldpolicy:
                ...
                with self.runtime_profiler.profile("old_log_prob", {"step": int(self.make_exp_step)}):
            ...
            with self.runtime_profiler.profile("add_experience", {"step": int(self.make_exp_step)}):
        ...
        self.runtime_profiler.record_step(self.make_exp_step)
