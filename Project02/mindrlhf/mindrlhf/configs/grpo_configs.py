...
class RLConfig:
    ...
    # segment rollout knobs
    enable_segment_rollout: bool = False
    global_max_decode_len: int = 0
    max_segment_count: int = 1
    segment_schedule: list = None
    importance_sampling_mode: str = "legacy"
...
class RuntimeProfileConfig:
    ...
    record_host_memory: bool = False
    step_log_interval: int = 0  # 0 disables periodic step logging
    epoch_log_interval: int = 0  # 0 disables periodic epoch logging
...
class GRPOConfig:
    ...
    runtime_profile_config = RuntimeProfileConfig
    ...
    def _update_sub_config(self, data):
        ...
        if "runtime_profile_config" in data:
            self.runtime_profile_config = RuntimeProfileConfig(**data["runtime_profile_config"])
