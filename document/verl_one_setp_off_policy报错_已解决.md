...
```shell
# patch下载链接：https://gitcode.com/Ascend/MindSpeed-RL/blob/abd4c8ce4b0e3ba7554f971a5505f32c36267b5d/verl_npu/verl_npu/patch/ray/ray_2_51_1_hccl.patch
...
在recipe/one_step_off_policy/ray_trainer.py脚本下，` def _create_weight_sync_group(self):`方法（271行左右）修改为：
...
```py
    ...
    def _create_weight_sync_group(self):
        ...
        # 主要修改了下面部分
        from ray.experimental import collective
...
4. **grpo_qwen3_8b_gsm8k_fsdp2_8_8_npu.sh**
...
### 附录1 如果不修改ray_trainer.py的代码 会报错 
...
解决方法，在recipe/one_step_off_policy/ray_trainer.py脚本下，` def _create_weight_sync_group(self):`方法（271行左右）修改为：
